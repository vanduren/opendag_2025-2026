(function (global, factory) {
    ...
    ***************************************************************************** */
    ...
    var FaceExpressions = /** @class */ (function () {
        function FaceExpressions(probabilities) {
            ...
            FACE_EXPRESSION_LABELS.forEach(function (expression, idx) {
                ...
                if (probabilities[idx] > 0.8) {
                    // *********************************************
                    // console.log(expression, probabilities[idx]);
                    // switch met cases voor het aan en uitzetten van licht
                    // op basis van moods
                    switch (expression) {
                        case "sad":
                            console.log('sad');
                            // fetch('http://192.168.0.101/H');
                            getArduinoH();
                            break;
                        case "happy":
                            console.log('happy');
                            // fetch('http://192.168.0.101/L');
                            getArduinoL();
                            break;
                        case "angry":
                            console.log('angry');
        ...
        FaceExpressions.prototype.asSortedArray = function () {
            ...
            return FACE_EXPRESSION_LABELS
                // .map(function (expression) { console.log('expression' + expression, 'probability' + _this[expression]);return ({ expression: expression, probability: _this[expression] }); })
    ...
    // functions voor arduino HTTP webserver
    // De H en L zorgen voor aan en uitzetten van licht
    async function getArduinoH() {
        const response = await fetch("http://192.168.0.101/H");
    }

    async function getArduinoL() {
        const response = await fetch("http://192.168.0.101/L");
    }
